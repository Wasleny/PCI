...
int calculaIdade(int mesNascimento, int anoNascimento)
{
    ...
    dataNascimento.tm_mday = 1;      // Dia de referência (exemplo: 1)

    // Calcular a diferença em anos
    int anos = dataAtual->tm_year - dataNascimento.tm_year;

    return dataNascimento.tm_mon < dataAtual->tm_mon ? anos - 1 < 5 : anos < 5;
}
